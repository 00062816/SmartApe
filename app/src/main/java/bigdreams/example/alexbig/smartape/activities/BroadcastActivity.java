...
import bigdreams.example.alexbig.smartape.utils.JsonConverter;
...
public class BroadcastActivity extends AppCompatActivity implements DataReceivedListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(bigdreams.example.alexbig.smartape.R.layout.broadcast_activity);

        quizViewModel = ViewModelProviders.of(this).get(QuizViewModel.class);
        wiFiDirectBroadcastReceiver = WiFiP2PInstance.getInstance(this).getBroadcastReceiver();

        listView = findViewById(bigdreams.example.alexbig.smartape.R.id.broadcast_groupListView);
        FloatingActionButton createGroupButton = findViewById(bigdreams.example.alexbig.smartape.R.id.broadcast_addGroupButton);
        FloatingActionButton findGroupButton = findViewById(bigdreams.example.alexbig.smartape.R.id.broadcast_findGroupButton);
    ...
    private void createGroup(Quiz quiz){
        ...
        wroupService.registerService(quiz.getTitle(), new ServiceRegisteredListener() {
            ...
            public void onSuccessServiceRegistered() {
                Toaster.makeToast(getApplicationContext(), getString(bigdreams.example.alexbig.smartape.R.string.text_group_created));
    ...
    private void findGroups(){
        ...
        progressDialog.setMessage(getString(bigdreams.example.alexbig.smartape.R.string.text_searching));
        progressDialog.show();
        ...
        wroupClient.discoverServices(5000L, new ServiceDiscoveredListener() {
            ...
            public void onFinishServiceDeviceDiscovered(List<WroupServiceDevice> serviceDevices) {
                ...
                if (serviceDevices.isEmpty()) {
                    Toaster.makeToast(getApplicationContext(), getString(bigdreams.example.alexbig.smartape.R.string.text_no_groups_found));
                }
            }

            @Override
            public void onError(WiFiP2PError wiFiP2PError) {
               Toaster.makeToast(getApplicationContext(), getString(bigdreams.example.alexbig.smartape.R.string.text_error_groups));
            }
        });
    }
    ...
    private void connectTo(WroupServiceDevice serviceDevice){
        ...
        progressDialog.setMessage(getString(bigdreams.example.alexbig.smartape.R.string.text_connecting));
        progressDialog.setIndeterminate(true);
        progressDialog.show();
        ...
        wroupClient.connectToService(serviceDevice, new ServiceConnectedListener() {
            ...
            public void onServiceConnected(WroupDevice serviceDevice) {
                progressDialog.dismiss();
                Toaster.makeToast(getApplicationContext(), getString(bigdreams.example.alexbig.smartape.R.string.text_connected));
            }
        });
    }
